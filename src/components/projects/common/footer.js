...
class Footer extends React.Component {
  constructor(props) {
    super(props)
    this.state = {linkedProjects: []}
  }
  componentDidMount() {
    const {project} = this.props
    this.setState({
      linkedProjects: isMobile() ? sampleSize(project.linkedProjects, 2) : project.linkedProjects
    })
  }
  render() {
    const { project, background = '#001732', projectsBackground = 'linear-gradient(to right, #61daff 0%, #941c91 100%)'} = this.props
    const {linkedProjects} = this.state
    return (
      <div style={{background}} className='Footer'>
       <style jsx>{`
        ...
        <div className='Projects'>
          {
            linkedProjects.map(index =>
              <div className='Project clickable' key={index}>
                <div style={{
                  top: 130
                }} className='Project_title playfairdisplay_black'>{data.projects[index].title}</div>
                <div className='Project_card transitions'>
                  <div className='Project_container'>
                    <div className='Project_card_title playfairdisplay_black' style={{
                      marginBottom: 20
                    }}>
